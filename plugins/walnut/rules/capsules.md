...
**Reference capsules** — accumulate related context. Goal describes a collection ("Collect and index all Nova Station call transcripts"). Companion IS the value — synthesizes what's in raw/. Status stays `active` (ongoing) or `done` (retired). Never graduates. Lives in `_core/_capsules/` permanently.
...
Capsules in `draft` status unchanged for 30+ days are surfaced by `walnut:system-cleanup`:
