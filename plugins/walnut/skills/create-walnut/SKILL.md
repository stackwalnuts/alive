...
Not a setup wizard (that's `world/setup.md` — first-time only). Not opening an existing walnut (that's `walnut:load-context`).
...
Capsule scaffolding is handled by `walnut:capsule-manager`. When the human says "create a capsule" or when stash routing identifies a new body of work, invoke the capsule skill — it handles the full create flow including companion template, raw/ directory, and now.md update.
