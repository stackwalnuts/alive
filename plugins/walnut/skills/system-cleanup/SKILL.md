...
Not a dashboard (that's world). Not a search (that's find). Not session history (that's walnut:session-history). Pure maintenance.
...
│
│  → route via walnut:capture-context / skip
╰─
```
...
- **Undeclared connections** — `[[wikilinks]]` used in body text or log entries but NOT in the frontmatter `links:` field. These are invisible to `walnut:search-world` traversal.
...
│  3. Move AGENTS.md → glass-cathedral/_core/_capsules/_inbox/raw/,
...
Also scan the walnut's live context (folders outside `_core/`) for legacy naming conventions from pre-walnut systems (e.g. directories using old naming like `_brain/` instead of `_core/`, `_working/`, `_references/`).
...
│     [[orion-vex]] to stellarforge `_core/key.md` frontmatter links: field
