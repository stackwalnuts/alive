...
If previous next: was NOT completed and is being replaced, it moves to `_core/tasks.md` with context.

### 4. Write Log Entry

**Before writing anything else, prepend a signed entry to `_core/log.md`.** This is the primary record of what happened. The log entry uses the standard template:
...
**The log entry must be written BEFORE updating `_core/now.md`. The log is truth. Everything else derives from it.**

### 5. Prepare Remaining Content (in memory)

**Re-read `_core/log.md` first ~150 lines** to ground the `_core/now.md` synthesis in the actual written log. This captures the entry just prepended in step 4 plus the previous 3-4 entries — enough for a proper 3-5 entry synthesis. Don't rely on memory of what was read in step 1; the log has changed since then.

Then prepare the content for all remaining files in memory:

- **_core/now.md** — full replacement: phase, capsule, next, updated, squirrel, context paragraph. Set `capsule:` to the active capsule name (or null if none). The context paragraph synthesises the last 3-5 log entries (including the one just written) — what's been happening across sessions, not just this session. A new squirrel reading `_core/now.md` should understand the full current situation without touching the log.
- **_core/tasks.md** — new tasks added, completed marked, in-progress updated
- **Cross-walnut dispatches** — brief log entries for destination walnuts
- **_core/insights.md** — new evergreen entries (only if confirmed in step 3)

**Protect existing `_core/now.md` context.** If this session was minor but the existing `_core/now.md` has rich context from a previous deep session — do NOT flatten it. Merge new information in. The test: is the new `_core/now.md` MORE informative than the old one? If not, keep what was there and layer the new stuff on top.
...
Parallel writes:
- `_core/now.md` — full replacement with prepared content
- `_core/tasks.md` — updates with prepared content
- `_core/insights.md` — new evergreen entries (if any confirmed)
...
Write the routed stash to the session's squirrel YAML in `.walnut/_squirrels/{session_id}.yaml`. This turns the YAML from a skeleton into an actual session record.
...
- [ ] **_core/now.md** — does the context paragraph reflect the full current picture (not just this session)?
- [ ] **Log entry** — does it capture WHY decisions were made, not just WHAT?
- [ ] **_core/tasks.md** — are new tasks added, completed tasks marked, nothing stale left as active?
- [ ] **Capsules** — was any capsule worked on this session? Is its companion updated (sources, decisions, status)?
- [ ] **References** — was any external content discussed this session that wasn't captured? Any research worth saving? (Route to capsule `raw/` if active capsule exists.)
...
│  zero-context: ✓
│
│  Run walnut:system-cleanup? (stale walnuts, orphan refs, stale drafts)
╰─
```

The check suggestion is lightweight — one line. If the human ignores it, no friction. If they say "check" or "yeah", invoke `walnut:system-cleanup`.
...
- Update the squirrel entry in `.walnut/_squirrels/{session_id}.yaml`:
