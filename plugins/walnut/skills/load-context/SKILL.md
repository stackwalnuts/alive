...
If the human picks "start something new" → invoke `walnut:capsule-manager` (create operation).
...
If yes → invoke `walnut:capsule-manager` (graduate operation).
...
The default is single-walnut focus. But `walnut:load-context walnut-a walnut-b` is valid for cross-walnut sessions:
