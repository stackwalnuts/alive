...
Not a list (that's the tree in walnut:world). Not a search (that's walnut:search-world). Map is spatial — it shows how everything connects and where the energy is.
...
**Plugin source:** Both scripts ship with the plugin at `scripts/generate-index.py` and `scripts/generate-graph.py`. On first `walnut:my-context-graph` invocation, copy them to `.walnut/scripts/` if not already present.
...
- On every `walnut:my-context-graph` invocation (always fresh)
- Suggested after `walnut:save` when structural changes occurred (new walnut, new capsule, new person)
- After `walnut:system-cleanup` resolves issues that affect the graph (broken links, orphan walnuts)
...
- Not `walnut:search-world` — find retrieves specific content. Map shows the topology.
- Not `walnut:system-cleanup` — tidy fixes structural issues. Map visualizes the structure.
