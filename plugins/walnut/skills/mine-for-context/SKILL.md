...
- `walnut:session-history` escalates — "these sessions look rich, want to deep-mine them?"
...
## Mining Sessions (via walnut:session-history)

When `walnut:session-history` identifies unmined sessions — long transcripts with extensive decision-making or research — it can hand off to mine with a specific scope:
...
The squirrel resolves transcript paths using the same discovery mechanism as history (see `walnut:session-history` Transcript Discovery section).
...
- Not `walnut:capture-context` — capture brings new content IN. Mine extracts value from content already captured.
- Not `walnut:session-history` — history shows the session timeline. Mine goes deep on specific sources.
- Not `walnut:search-world` — find searches for known things. Mine discovers unknown things.
