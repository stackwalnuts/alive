...
Not about adjusting preferences or voice (that's `walnut:settings`). Extend is about creating NEW things — skills, rules, hooks — that make the system do something it couldn't before.
...
│  Fires: after every email capture via walnut:capture-context
...
```
╭─ 🐿️ this skill could work for other builders
...
- Not `walnut:settings` — tune adjusts preferences and config. Extend creates new capabilities.
