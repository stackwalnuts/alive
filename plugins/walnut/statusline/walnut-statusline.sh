...
if [ "$COST" = "\$0.00" ]; then
  ...
  TIPS=(
    ...
    "${DIM}/walnut:load-context${RESET} ${DIM}— lock in on one walnut${RESET}"
    "${DIM}/walnut:save${RESET} ${DIM}— checkpoint, compound context${RESET}"
    "${DIM}/walnut:system-cleanup${RESET} ${DIM}— clean your world${RESET}"
    "${DIM}/walnut:capture-context${RESET} ${DIM}— bring context in${RESET}"
    "${DIM}/walnut:search-world${RESET} ${DIM}— search everything${RESET}"
    "${DIM}/walnut:session-history${RESET} ${DIM}— recent session context${RESET}"
    "${DIM}/walnut:mine-for-context${RESET} ${DIM}— deep context extraction${RESET}"
  )
